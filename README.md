...
- Проект остановлен и переведен в режим поддержки архива (новая разработка не ведется).
- Причина остановки: фундаментальное ограничение Yandex Calendar для сценария двусторонней синхронизации участников встреч.
- Через CalDAV можно передавать `VEVENT`, но в пользовательском сценарии Яндекса поле «Участники» не гарантированно синхронизируется как нативная сущность между системами.
- Из-за этого нельзя надежно выполнить ключевой бизнес-сценарий: `Bitrix -> Yandex -> Bitrix` с консистентным автоматическим переносом/обновлением участников.
...
- Участники встреч (`ATTENDEE`) не считаются надежно синхронизируемыми между Bitrix24 и нативным полем «Участники» в Yandex Calendar; это продуктово-протокольное ограничение провайдера в данном интеграционном сценарии.
